...
export async function parseDataFile(filePath) {
  ...
  if (!context.window || !context.window.WORD_ATELIER_DATA) {
    throw new Error(`window.WORD_ATELIER_DATA introuvable dans ${filePath}`);
  }
  return { kind: "js", hadBom: hasBom(raw), raw: text, data: context.window.WORD_ATELIER_DATA };
}

export function serializeDataFile(kind, dataObject) {
  const json = JSON.stringify(dataObject, null, 2);
  return kind === "js" ? `window.WORD_ATELIER_DATA = ${json};\n` : `${json}\n`;
}
