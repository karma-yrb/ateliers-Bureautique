...
const EXERCISE_IMAGE_HINT = /\/images\/(02-word|03-word|03-powerpoint|powerpoint|word|tableau|publipostage|document|exercices|niveau)/i;
...
function findDocxUrl(html) {
  const matches = [...html.matchAll(/https?:\/\/[^"' >]+\.(?:pptx|xls|xlsm|zip|docx)/gi)].map((m) => m[0]);
...
async function buildCatalogFromIndex() {
  ...
  while ((headingMatch = h3Regex.exec(articleHtml)) !== null) {
    ...
    if (/^(PowerPoint|Fonctions|Pr[ée]parez|Exercices complets)/i.test(title)) {
    ...
    while ((moduleMatch = level1Regex.exec(sectionHtml)) !== null) {
      ...
      while ((linkMatch = linkRegex.exec(liHtml)) !== null) {
        ...
        moduleExercises.push({
          id: `powerpoint-ex-${String(globalIndex).padStart(3, "0")}`,
          globalIndex,
          moduleId,
          moduleName,
