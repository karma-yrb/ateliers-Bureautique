...
function buildOpenWorkbookStep() {
  return 'Ouvrez Microsoft PowerPoint : cliquez sur le bouton Demarrer, recherchez "PowerPoint", puis ouvrez l\'application. Ensuite, cliquez sur "Presentation vierge" pour creer un nouveau classeur, puis enregistrez-le avant de commencer.';
}

function startsWithOpenWorkbookStep(step) {
  const text = normalizeTitleKey(step);
  return (
    text.includes("ouvrez microsoft powerpoint") ||
    text.includes("classeur vierge") ||
    text.includes("nouveau classeur") ||
    text.includes("vous partez d'un nouveau classeur") ||
    text.includes("creez un nouveau classeur powerpoint")
  );
}
...
function getContextualAction(exercise) {
  ...
  return "Appliquez les outils PowerPoint utiles (saisie, formules, mise en forme, donnees ou insertion) pour obtenir le rendu attendu.";
}
