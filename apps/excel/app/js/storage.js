class WordAtelierFileStorage {
  constructor() {
    this.dbName = "excel_atelier_fs_settings_v1";
  ...
  async openUserDirectory(startHandle = null) {
    return window.showDirectoryPicker({
      id: "excel-atelier-user-folder-open",
      mode: "readwrite",
  ...
  async pickDocumentsDirectory() {
    return window.showDirectoryPicker({
      id: "excel-atelier-documents-root",
      mode: "read",
      startIn: "documents",
    });
  }
  ...
  async loadProgress(rootHandle, initials) {
    ...
    let fileHandle = null;
    try {
      fileHandle = await progressDir.getFileHandle("progression-excel.json");
    } catch {
      return null;
    }
  ...
  async saveProgress(rootHandle, initials, progressObject) {
    const progressDir = await this.ensureProgressDirectory(rootHandle, initials, true);
    const fileHandle = await progressDir.getFileHandle("progression-excel.json", { create: true });
  ...
  getProgressRelativePath(initials) {
    return "ProgressionAtelier/progression-excel.json";
  }
  ...
  async getSavedExerciseFiles() {
    ...
    for (const item of raw) {
      ...
      items.push({
        ...
        exerciseId,
        handle,
        fileName: String(item.fileName || handle.name || "fichier.xlsx").trim() || "fichier.xlsx",
        lastUsedAt: typeof item.lastUsedAt === "string" ? item.lastUsedAt : "",
      });
    }
    return items;
  ...
  async setSavedExerciseFiles(items) {
    ...
    const payload = items
      ...
      .map((item) => ({
        ...
        handle: item.handle,
        fileName: String(item.fileName || item.handle.name || "fichier.xlsx").trim() || "fichier.xlsx",
        lastUsedAt: typeof item.lastUsedAt === "string" ? item.lastUsedAt : "",
      }))
  ...
  async setSavedExerciseFile(profileKey, exerciseId, handle) {
    ...
    next.push({
      ...
      handle,
      fileName: handle.name || "fichier.xlsx",
      lastUsedAt: now,
    });
