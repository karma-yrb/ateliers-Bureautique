...
class ExcelAtelierController {
  ...
  async #loadProgressForSession(session) {
    ...
    this.view.setProgressUserPath(`Fichier: ${rootName} > ProgressionAtelier > progression-excel.json`);
  }
  ...
  async #trackExerciseDownloadFromLink(linkEl) {
    ...
    await this.#refreshExerciseWorkFileState(exerciseId, {
      statusText: `Téléchargement lancé. Ouvrez le document dans Excel, cliquez sur « Activer la modification », enregistrez ${fileName} dans votre dossier utilisateur, puis cliquez sur « Ouvrir mon fichier ».`,
    });
  }
  ...
  #getSaveReminderFileName(exerciseId) {
    if (!exerciseId) return "exercice-termine.xlsx";
    const normalized = String(exerciseId).trim().toLowerCase();
    if (/^ex-\d+$/.test(normalized)) {
      return `${normalized}-termine.xlsx`;
    }
    const exercise = this.model.getExerciseById(exerciseId);
    if (!exercise || typeof exercise.num !== "number") return "exercice-termine.xlsx";
    return `ex-${String(exercise.num).padStart(3, "0")}-termine.xlsx`;
  }
